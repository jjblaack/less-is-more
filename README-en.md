...
> **Less is more. Your judgment is the steering wheel. AI is the sports car.**

A personal AI development harness for **Claude Code** — two paths, one philosophy: **the minimum number of constraints that still keep you in control**.
...
Claude Code is powerful. But without a workflow, it's hard to get high-quality output in a single pass. And with *too many* rules, you end up with stale docs and rigid processes that slow you down.

Most harnesses solve this by adding **more** — more rules, more phases, more guardrails. We go the other direction: **subtract**. More importantly, knowing *where* to add and *where* to subtract matters even more.

Six iron rules. Three phases for non-trivial work. Three safety hooks. Every constraint earns its place by either protecting you directly or multiplying the agent's output. Everything else is noise.
...
> A quick note: my view is that a harness is never a paradigm. It's a philosophy. The word says it all: put on the saddle, then ride free.

**The core loop for better code generation:**
- **Think first** — the agent restates your ask, waits for your "yes", proposes a plan, waits again. No drive-by commits. When your cognition and the agent's align, you're pointed at the same target.
- **Structure when it matters** — trivial? Go direct. Non-trivial? `/plan` → `/code` → `/evaluate`. No ceremony for simple stuff.
- **Safety on by default** — secrets blocked, destructive commands need your go-ahead.
- **Gets smarter on its own** — end of every session, a prompt pops up automatically (zero commands to run) asking you to capture lessons learned.
...
Or just tell Claude: "I want to install this harness. Walk me through it based on the files in this repo."
